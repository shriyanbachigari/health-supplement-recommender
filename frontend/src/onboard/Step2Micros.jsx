import { MICRONUTRIENT_OPTIONS } from './constants'
import Navbar from '../components/NavBar'

export default function Step2Micros({ selected, onToggle, onNext }) {
  const handleCheckboxChange = (microKey) => {
    onToggle(microKey)
  }
            ...
            <div className="layout-content-container flex flex-col w-[1100px] max-w-[1100px] py-5 flex-1">
              <div className="flex flex-col gap-3 p-4">
                <div className="flex gap-6 justify-between">
                  <p className="text-[#0e151b] text-base font-medium leading-normal">Step 2 of 4</p>
                </div>
                <div className="rounded bg-[#d0dde7]">
                  <div className="h-2 rounded bg-[#47a6ea]" style={{ width: '50%' }}></div>
                </div>
              </div>
              <h2 className="text-[#0e151b] tracking-light text-[28px] font-bold leading-tight px-4 text-center pb-3 pt-5">
                Which micronutrients are you interested in?
              </h2>
              <p className="text-[#0e151b] text-base font-normal leading-normal pb-3 pt-1 px-4 text-center">
                Select the primary ones you'd like to focus on.
              </p>
              <div className="px-4 max-h-96 overflow-y-auto">
                {MICRONUTRIENT_OPTIONS.map((micro) => (
                  <label key={micro.key} className="flex gap-x-3 py-3 flex-row">
                    <input
                      type="checkbox"
                      checked={selected.includes(micro.key)}
                      onChange={() => handleCheckboxChange(micro.key)}
                    ...
                    />
                    <p className="text-[#0e151b] text-base font-normal leading-normal">{micro.label}</p>
                  </label>
                ))}
              </div>
